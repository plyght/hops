...
public struct OCIImageConfig: Codable, Sendable {
    public struct Config: Codable, Sendable {
        ...
        public init(Env: [String]?, Cmd: [String]?, WorkingDir: String?, Entrypoint: [String]?, User: String?) {
            self.Env = Env
            self.Cmd = Cmd
...
public actor OCIImageManager {
    ...
    public init(cacheDirectory: URL? = nil) {
        ...
        self.cacheDirectory = cacheDirectory ?? homeDir
    ...
    public func resolveImage(_ imageString: String) async throws -> URL {
        ...
        let layers = try await downloadLayers(imageRef: imageRef, manifest: manifest, cacheDir: imageCacheDir)
    ...
    private func fetchManifest(imageRef: OCIImageReference) async throws -> OCIManifest {
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            throw OCIImageError.fetchFailed("Failed to fetch manifest: HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0)")
        }
        
        return try JSONDecoder().decode(OCIManifest.self, from: data)
    }
    
    private func downloadLayers(imageRef: OCIImageReference, manifest: OCIManifest, cacheDir: URL) async throws -> [URL] {
        ...
        for (index, layer) in manifest.layers.enumerated() {
            ...
            if !FileManager.default.fileExists(atPath: layerPath.path) {
                ...
                guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                    throw OCIImageError.fetchFailed("Failed to download layer: HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0)")
                }
        ...
        if !FileManager.default.fileExists(atPath: configPath.path) {
            ...
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                throw OCIImageError.fetchFailed("Failed to download config: HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0)")
            }
    ...
    private func convertToExt4(tarPath: URL, outputPath: URL) async throws -> URL {
        ...
        guard extractProcess.terminationStatus == 0 else {
            let _ = Process()
            try? Process.run(URL(fileURLWithPath: "/usr/bin/sudo"), arguments: ["umount", mountPoint.path])
