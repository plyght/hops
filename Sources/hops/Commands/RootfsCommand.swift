...
extension RootfsCommand {
  ...
  struct CreateRootfs: AsyncParsableCommand {
    ...
    func run() async throws {
      ...
      try unpacker.unpack(
        archive: tarballPath, compression: ContainerizationArchive.Filter.gzip, at: rootfsPath)
  ...
  struct DeleteRootfs: AsyncParsableCommand {
    ...
    private func checkLastRootfs(at directory: URL, deletingName: String) throws {
      ...
      if rootfsImages.count == 1
        && rootfsImages.first?.deletingPathExtension().lastPathComponent == deletingName {
        throw ValidationError(
          """
    ...
    private func checkProfileReferences(rootfsName: String) throws -> [String] {
      let profileDir = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(
        ".hops/profiles")
